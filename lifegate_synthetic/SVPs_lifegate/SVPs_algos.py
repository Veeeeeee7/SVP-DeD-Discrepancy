...
# computes Q from V and then near-greedy policy with respect to Q
# one-step exploration of states to update V using of worst-case among the near-greedy actions
# recompute Q and near-greedy policy after each full sweep of states
# converges when the last two policies are the same and delta < theta
def value_iter_near_greedy(env, gamma, zeta, V_star=None, window=10, theta=1e-10, max_iter=1000):
    ...
    # compute the state-action function Q from state-value function V
    Q = V2Q(env, V, gamma)
    Pi = np.zeros_like(Q)

    # builds a thresholded policy (boolean mask on state-action pairs) where actions are included if their Q-value is within (1 - zeta) of V*
    for s in range(env.nS):
    ...
    # while not converged 
    while True:
        # delta for convergence check
        delta = 0.0
        
        # for each step update value function
        for s in range(env.nS):
            old_v = V[s]
            ...
            if V_star[s] > 0:
                # creates threshold for near-greedy beneficial actions
                Q_cutoff = (1 - zeta) * V_star[s]  # lower bound for return
            else:
                # creates threshold for near-greedy harmful actions
                Q_cutoff = V_star[s] - zeta * abs(V_star[s])

            Pi_S = np.argwhere(Q_s >= Q_cutoff)

            # update value function using worst-case among the near-greedy actions or greedy action set is empty
            ...
            # compute max change of value function across all states for convergence check
            delta = max(delta, np.abs(new_v - old_v))
        ...
        # convergence check: compare last two policies and delta
